...
*
* If a subproject declares a libraryDependency which corresponds to another subproject's ID, replace the libraryDependency
* with a source dependency instead.
*
* This allows having a standalone library repository that is fully buildable and publishable on its own (via binary dependencies),
* and including the same repository as e.g. a git submodule as part of a larger project without having to go through
* a maven or Ivy repository when iterating on the library in the context of the large project.
*/
